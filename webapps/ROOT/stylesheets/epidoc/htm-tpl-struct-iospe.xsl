  ...
  <xsl:template name="iospe-body-structure">
    ...
    <div id="text-field">
      ...
      <dl class="iospe">
        ...
        <dd>
          <xsl:choose>
            <xsl:when test="//t:layout/t:dimensions//text()[not(normalize-space(.)=' ')]">
              <xsl:if test="//t:layout/t:dimensions/t:height/text()[not(normalize-space(.)=' ')]">H. <xsl:value-of select="//t:layout/t:dimensions/t:height"/>,</xsl:if>
              <xsl:if test="//t:layout/t:dimensions/t:width/text()[not(normalize-space(.)=' ')]">W. <xsl:value-of select="//t:layoit/t:dimensions/t:width"/>,</xsl:if>
            </xsl:when>
            <xsl:otherwise><i18n:text i18n:key="epidoc-xslt-iospe-unknown">Unknown</i18n:text></xsl:otherwise>
          </xsl:choose>
        </dd>
      ...
      <!--  
       ...
       -->   
        ...
        <dt width="150" align="left"><i18n:text i18n:key="epidoc-xslt-iospe-handnote">Script, style</i18n:text></dt> 
        <dd>
          <xsl:choose>
            <xsl:when test="//t:handNote/text()">
              <xsl:value-of select="//t:handNote"/>
            </xsl:when>
            <xsl:otherwise><i18n:text i18n:key="epidoc-xslt-iospe-not-specified">Not specified</i18n:text></xsl:otherwise>
          </xsl:choose>
        </dd>
        
        <dt width="150" align="left"><i18n:text i18n:key="epidoc-xslt-iospe-deconote">Decoration</i18n:text></dt> 
        <dd>
          <xsl:choose>
            <xsl:when test="//t:decoNote/text()">
              <xsl:value-of select="//t:decoNote"/>
            </xsl:when>
            <xsl:otherwise><i18n:text i18n:key="epidoc-xslt-iospe-not-specified">Not specified</i18n:text></xsl:otherwise>
          </xsl:choose>
        </dd>
