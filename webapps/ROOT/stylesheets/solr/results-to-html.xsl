...
<xsl:stylesheet exclude-result-prefixes="#all" version="2.0"
  ...
  <xsl:template match="lst[@name='facet_fields']" mode="search-results">
    <xsl:if test="lst/int">
      <h3><i18n:text i18n:key="facets">Facets</i18n:text></h3>
  ...
  <xsl:template match="response/result" mode="search-results">
    <xsl:choose>
      <xsl:when test="number(@numFound) = 0">
        <h3><i18n:text i18n:key="no-result">No results found</i18n:text></h3>
      </xsl:when>
      <xsl:when test="doc">
        <ul>
  ...
  <xsl:template match="*[@name='fq']" mode="search-results">
    <h3><i18n:text i18n:key="filtered">Current filters</i18n:text></h3>
